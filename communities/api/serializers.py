...
class CommunityCreateSerializer(serializers.ModelSerializer):
  ...
  def save(self):
    ...
    try:
      ...
      if not is_image_aspect_ratio_valid(url):
        os.remove(url)
        raise serialziers.ValidationError({"response": "Image height must not exceed image width. Try a different image."})

      os.remove(url)
      community.save()
      return community
    except KeyError:
      raise serializers.ValidationError({"response": "You must have name, backgroundimage, avatarimage and description for community."})
